...
Graph::Graph () { 
	...
	window.create(VideoMode(800, 520), "Graph Drawer");
	...
	ConnectionFont.loadFromFile("Fonts/Terminus-Bold.ttf");
	NodeFont.loadFromFile("Fonts/Terminus-Bold.ttf");
}
	...
	if (!inputBox.isActive()) {
		...
		if (NodeHit) {
			if (!ConnectionIsActive) {
				...
				connections.push_back(new Connection(selectedNode, ConnectionFont));
			} else {
				ConnectionIsActive = false;
			...
			if (!ConnectionIsActive) {
				nodes.push_back(new Node(mouse_position, nodes.size()+1, NodeFont));
			} else {
				ConnectionIsActive = false;
