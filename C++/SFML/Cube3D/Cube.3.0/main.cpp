...
public:
	...
	Camera (Vector3f position, Vector2f rotation = Vector2f(0.f, 0.f)) {
...
public:
	...
	void draw (Camera *camera) {
		for (int i = 0; i < indexBuffer.size(); i++) {
			if (convert_rotation(vertexBuffer[indexBuffer[i].x],
					camera->getPosition(), camera->getRotation()).z > 0 || 
				convert_rotation(vertexBuffer[indexBuffer[i].y],
					camera->getPosition(), camera->getRotation()).z > 0) {
				line[0].position = convert_position(convert_rotation(vertexBuffer[indexBuffer[i].x],
					camera->getPosition(), camera->getRotation()))+center;
				line[1].position = convert_position(convert_rotation(vertexBuffer[indexBuffer[i].y],
					camera->getPosition(), camera->getRotation()))+center;
				/*if ((line[0].position.x > 0 && line[0].position.x < window->getSize().x && 
					line[0].position.y > 0 && line[0].position.y < window->getSize().y) ||
					(line[1].position.x > 0 && line[1].position.x < window->getSize().x && 
					line[1].position.y > 0 && line[1].position.y < window->getSize().y))*/
					window->draw(line);
			}
		}
	}
...
int main() 
{
	...
	scene.addCamera(new Camera(Vector3f(-90.3737, -16, 35.7612)));
	...
	scene.addObject(new Cube3D(Vector3f(0.f, -3.f, 0.f), 0.5f));
	scene.addObject(new Cube3D(Vector3f(0.f, 0.f, 3.f)));
	scene.addObject(new Cube3D(Vector3f(0.f, 0.f, -3.f), 3.f));*/
	scene.setCamera(0);
